...
describe("routes : comments", () => {
  ...
  beforeEach((done) => {
    ...
    sequelize.sync({force: true}).then((res) => {
      ...
      User.create({email: "starman@tesla.com", password: "Trekkie4lyfe"}).then((user) => {
        ...
        Topic.create({
          ...
          Comment.create({body: "ay caramba!!!!!", userId: this.user.id, postId: this.post.id}).then((coment) => {
  ...
  describe("guest attempting to perform CRUD actions for Comment", () => {
    ...
    describe("POST /topics/:topicId/posts/:postId/comments/:id/destroy", () => {
      ...
      it("should not delete another user's comment", (done) => {
        User.create({email: "jayesh@jayjay.com", password: "123456"}).then((user) => {
          expect(user.email).toBe("jayesh@jayjay.com");
