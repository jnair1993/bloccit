...
describe("Favorite", () => {
  ...
  beforeEach((done) => {
    ...
    sequelize.sync({force: true}).then((res) => {
      ...
      User.create({email: "starman@tesla.com", password: "Trekkie4lyfe"}).then((res) => {
        this.user = res;
        ...
        Topic.create({
          ...
          Comment.create({body: "ay caramba!!!!!", userId: this.user.id, postId: this.post.id}).then((res) => {
            this.comment = res;
            done();
          }).catch((err) => {
            console.log(err);
  ...
  describe("#create()", () => {
    ...
    it("should create a favorite for a post on a user", (done) => {
      ...
      Favorite.create({postId: this.post.id, userId: this.user.id}).then((favorite) => {
    ...
    it("should not create a favorite without assigned post or user", (done) => {
      Favorite.create({userId: null}).then((favorite) => {
  ...
  describe("#getUser()", () => {
    ...
    it("should return the associated user", (done) => {
      Favorite.create({userId: this.user.id, postId: this.post.id}).then((favorite) => {
